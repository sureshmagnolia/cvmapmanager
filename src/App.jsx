...
function App() {
  ...
  if (printMode === 'paper-bundles') {
    return (
      <StandalonePrintWrapper title="Paper Bundles" orientation="portrait">
        <PaperBundlesPrint computedAllocations={computedAllocations} papers={papers} />
      </StandalonePrintWrapper>
    );
  }
...
function PaperBundlesPrint({ computedAllocations, papers }) {
  const paperBundles = {};
  computedAllocations.forEach(a => {
    if (a.actualCount > 0) {
  ...
  return (
    <div className="paper-bundles-print">
      <h2>Paper Bundles Overview</h2>
      <table className="print-table" style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ccc', marginTop: '1rem' }}>
        <thead>
          <tr>
            ...
            <th style={{ border: '1px solid #ccc', padding: '8px', textAlign: 'left', width: '35%' }}>Title of Paper</th>
            <th style={{ border: '1px solid #ccc', padding: '8px', textAlign: 'left', width: '50%' }}>Bundle numbers</th>
          </tr>
        </thead>
        <tbody>
          {Object.keys(papers).map(paperKey => {
